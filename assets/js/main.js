...
function onLogin() {
  ...
  for (let i = 0; i < arrCustomer.length; i++) {
    if (
      username.toUpperCase() == arrCustomer[i].user.toUpperCase() &&
      password == arrCustomer[i].password &&
      arrCustomer[i].status != "Bị khóa"
    ) {
      flag = 1;
      if (arrCustomer[i].level == 0) {
        level = arrCustomer[i].level;
        admin = true;
      }
      break;
    }
...
function onRegister() {
  ...
  if (username == "" || username.length < 5) {
    document.getElementById("errorUser").textContent =
      "Username phải lớn hơn 5 kí tự";
    flag = 0;
  }
  if (password == "" || password.length < 5) {
    document.getElementById("errorPass").textContent =
      "Password phải lớn hơn 5 kí tự";
    flag = 0;
  }
  if (email == "" || email.length < 5 || email.indexOf("@") == -1) {
    document.getElementById("errorEmail").textContent =
      "Email phải có @ và lớn hơn 5 kí tự";
    flag = 0;
  }
...
function showModal(id) {
  let arrModal = arrProduct.filter((product) => product.id == id);

  document.getElementById("modalProduct").style.display = "flex";
  let imgProduct = '<img src="./assets/img/' + arrModal[0].img + '" alt="">';
  document.getElementById("imgModalProduct").innerHTML = imgProduct;

  let nameProduct = '<h3 id="textModalProduct">' + arrModal[0].name + "</h3>";
  document.getElementById("nameModalProduct").innerHTML = nameProduct;

  let pricesProduct =
    "<p>Giá sản phẩm: <span>$" + arrModal[0].prices + "</span></p>";
  document.getElementById("pricesModalProduct").innerHTML = pricesProduct;

  document.getElementById("btnCart").innerHTML =
    '<input type="button" value="Thêm vào giỏ" id="addcart" onclick="addProductToCart(' +
    id +
    ')">';
  document.querySelector(".quatify").value = 1;
  document.querySelector(".btn__buynow").innerHTML =
    '<input type="button" value="Xem chi tiết" id="buynow" onclick="jumpToInfo(' +
    id +
    ')">';
}
...
function renderProduct(arrProduct) {
  let page = parseInt(getURL("page")) || 1;
  let perPage = 8;
  ...
  while (k < arrProductPag.length) {
    s =
      s +
      '<div class="col l-3 m-4 c-12"> <div class="home__product-item" onclick="showModal(' +
      arrProductPag[k].id +
      ')"><div class="home__product-item-img"> <img src="./assets/img/' +
      arrProductPag[k].img +
      '" alt=""> </div>' +
      ' <div class="home__product-item-description"><h4>' +
      arrProductPag[k].name +
      "</h4></div>" +
      ...
      "</div>" +
      '<div class="home__product-item-prices"><h4>$' +
      arrProductPag[k].prices +
      "</h4> </div>" +
...
function renderProductSearch() {
    ...
    document.getElementById("display_product").innerHTML = s
    document.querySelector('.img-pr').style.visibility = 'hidden'
    document.querySelector('.pagination__list').innerHTML = ''
  }

function renderDisplayProduct() {
  const arrSearch = arrProduct.filter(
    (product) => product.type == getURL("product")
  );
  getURL("product") != null
...
function renderVQMM() {
  ...
  if (event == "vqmm") {
    ...
    let s =
      '<h1 class="luckyheading">VÒNG QUAY</h1><div align="center" class=""><table >  <tr> <td><div class="power_controls">' +
      ...
      "</canvas></td></tr></table>" +
      '<div class="spinlucky"><input type="button" value="Quay" id="spin_button" onclick="startSpin();"></br></br>' +
      '<p onclick="resetWheel(); return false;" id="playagain">Chơi lại</p><br /></div>' +
      "</div>";
...
function alertPrize(indicatedSegment) {
  swal(
    ...
    "Mã giảm giá: " + indicatedSegment.keySale,
    "success"
  );
  localStorage.setItem(
    sessionStorage.getItem("user") + "sale",
    JSON.stringify(indicatedSegment)
  );
}

function renderPagination() {
  let product = getURL("product");
  let page = getURL("page") || 1;
  let prePage = 1;
  ...
  if (product != null) {
    document.getElementById("prePage").href =
      "?product=" + product + "&page=" + prePage;
    document.getElementById("nextPage").href =
      "?product=" + product + "&page=" + nextPage;
  } else {
    document.getElementById("prePage").href = "?page=" + prePage;
    document.getElementById("nextPage").href = "?page=" + nextPage;
  }
}

function renderContainerHeading() {
  getURL("product") != null
    ? (document.getElementById("containerHeading").textContent =
        getURL("product"))
    : (document.getElementById("containerHeading").textContent =
        "LATEST PRODUCTS");
}
...
function renderEmptyProduct() {
  document.getElementById("display_product").innerHTML =
    '<div id="errorSearch">Không tìm thấy sản phẩm</div>';
}
...
function searchInfo(id) {
  ...
  const type = getURL("product") || "";
  let arrSearchWithName = arrProduct.filter((product) =>
    type != ""
      ? product.name.toUpperCase().search(txtSearch.toUpperCase()) != -1 &&
        product.type.toUpperCase().search(type.toUpperCase()) != -1
      : product.name.toUpperCase().search(txtSearch.toUpperCase()) != -1
  );
  search = arrSearchWithName
  search.length > 7 ? document.querySelector('.pagination__list').innerHTML = `<button class="btnxemthem" onclick="renderProductSearch()">Xem thêm</button>` : document.querySelector('.pagination__list').innerHTML = ''
...
function sortAscending() {
  const type = getURL("product") || "";
  let arrType = arrProduct.filter((product) =>
    type != ""
      ? product.type.toUpperCase().search(type.toUpperCase()) != -1
      : product
  );
...
function searchPrices() {
  ...
  const arrFilter = arrProduct.filter((product) => {
    if (type != "") {
      return name != ""
        ? parseInt(product.prices) >= parseInt(start) &&
            parseInt(product.prices) <= parseInt(end) &&
            product.name.toUpperCase().search(name.toUpperCase()) != -1 &&
            product.type.toUpperCase().search(type.toUpperCase()) != -1
        : parseInt(product.prices) >= parseInt(start) &&
            parseInt(product.prices) <= parseInt(end) &&
            product.type.toUpperCase().search(type.toUpperCase()) != -1;
...
function filter2() {
  ...
  const arrFilter = arrProduct.filter((product) =>
    type != ""
      ? parseInt(product.prices) >= 1000 &&
        parseInt(product.prices) <= 2000 &&
        product.type.toUpperCase().search(type.toUpperCase()) != -1
      : parseInt(product.prices) >= 1000 && parseInt(product.prices) <= 2000
  );
  renderProduct(arrFilter);
}
...
//Cart
let arrMyProduct =
  localStorage.getItem(sessionStorage.getItem("user")) != null
  ...
  if (isLogin()) {
    ...
    swal({
      ...
      if (willDelete) {
        ...
        if (localStorage.getItem(userSession) != null) {
          for (var index in arrMyProduct) {
            if (arrMyProduct[index].id === arrProduct[id].id) {
              arrMyProduct[arrMyProduct.length - 1]["quantify"] =
                arrMyProduct[index].quantify + 1;
        ...
        swal(
          "Thêm thành công!",
          "Nếu bạn muốn thanh toán, vui lòng vào giỏ hàng!",
          "success"
        );
...
function onCart() {
  isLogin() ? (window.location = "cart.html") : displayLogin();
}
